...
class VideoProcessorTrack(VideoStreamTrack):
    ...
    # --- Analysis cadence ---------------------------------------------------
    # Controls WHEN frames are dispatched to the VLM. Default preserves the
    ...
    async def recv(self):
        ...
        try:
            ...
            # ---------------------------------------------------------------
            # Decide whether to dispatch this frame to the VLM based on the
            ...
            if should_dispatch:
                # Backpressure: skip if the previous VLM call is still
                # running. Without this, slow inference (esp. on CPU Ollama)
                # piles up tasks until something OOMs or hangs — which is
                # the "VLM stops responding" symptom emplus exhibits.
                ...
                if getattr(self.vlm_service, "is_processing", False):
                    logger.info(
                        ...
                        f"VLM still processing previous frame (mode={mode})"
                    )
                else:
                    ...
                    logger.info(
                        f"Frame {self.frame_count}: dispatched to VLM (mode={mode})"
                    )
